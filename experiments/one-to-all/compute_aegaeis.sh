...
if [ ! -d "$OUTPUT_DIR/triangle" ]; then
  compute_ota_queries "$TRIANGLE_TRIANGULATION_GRAPH" "$OUTPUT_DIR/triangle/inf/" "$QUERY_FILE" inf ""

  # refined graph with steiner points
  EPSILONS=("1.0" "0.5" "0.25" "0.125" "0.0625" "0.03125" "0.015625")
  for eps in "${EPSILONS[@]}"; do
    compute_ota_queries "$TRIANGLE_TRIANGULATION_GRAPH" "$OUTPUT_DIR/triangle/$eps/" "$QUERY_FILE" "$eps" ""
  done
fi
...
if [ ! -d "$OUTPUT_DIR/unref" ]; then
  compute_ota_queries "$UNREF_TRIANGULATION_GRAPH" "$OUTPUT_DIR/unref/inf/" "$QUERY_FILE" inf ""

  # unrefined graph with steiner points
  EPSILONS=("1.0" "0.5" "0.25")
  for eps in "${EPSILONS[@]}"; do
    compute_ota_queries "$UNREF_TRIANGULATION_GRAPH" "$OUTPUT_DIR/unref/$eps/" "$QUERY_FILE" "$eps" ""
  done
fi
