...
if [ ! -d "$OUTPUT_DIR/approximate-ref" ]; then
	...
	for eps in "${EPSILONS[@]}"; do
	    compute_ota "$TRIANGULATION_REF_GRAPH" "$OUTPUT_DIR/approximate-ref" "$QUERY_FILE" "$eps" ""
	done
fi
...
if [ ! -d "$OUTPUT_DIR/approximate-unref" ]; then
	...
	for eps in "${EPSILONS[@]}"; do
	    compute_ota "$TRIANGULATION_UNREF_GRAPH" "$OUTPUT_DIR/approximate-unref" "$QUERY_FILE" "$eps" ""
	done
fi
...
if [ ! -d "$OUTPUT_DIR/explicit-ref" ]; then
	compute_ota "$EXPLICIT_REF_GRAPH" "$OUTPUT_DIR/explicit-ref-05" "$QUERY_FILE" 0.0 ""
fi
