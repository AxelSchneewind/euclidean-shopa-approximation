...
def main():
    ...
    # by epsilon
    column_by_epsilon = [ data[data['epsilon'] == eps][args.column] for eps in data['epsilon'].unique() ]

    print('number of exact solutions: ')
    print(len(data[data['epsilon'] == math.inf]))

    # generate box plots
    fig, ax = plt.subplots()
    ax.boxplot(column_by_epsilon, labels=[eps for eps in data['epsilon'].unique()])
