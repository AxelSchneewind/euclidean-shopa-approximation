...
public:
    ...
    node_label(auto &&node, auto &&parent, auto &&distance, auto&& heuristic) {
        if constexpr (HasNode) {
            _impl._node = node;
        }
        if constexpr (HasDistance) {
        ...
        if constexpr (HasHeuristic) {
            _impl._heuristic = heuristic;
        }
        if constexpr (HasPredecessor) {
            _impl._predecessor = parent;
        }
        if constexpr (HasFaceCrossingPredecessor) {
            _impl._face_crossing_predecessor = parent;
        }
    }

    node_label(auto &&node, auto &&parent, auto &&distance, auto&& heuristic, auto face_crossing_predecessor) {
        if constexpr (HasNode) {
            _impl._node = node;
        }
        if constexpr (HasDistance) {
        ...
        if constexpr (HasFaceCrossingPredecessor) {
            _impl._face_crossing_predecessor = face_crossing_predecessor;
        }
    }


    [[gnu::hot]]
    node_label(auto &&node, auto &&parent, auto &&distance) {
        if constexpr (HasNode) {
            _impl._node = node;
        }
        if constexpr (HasDistance) {
    ...
    auto const &
    face_crossing_predecessor() const requires HasFaceCrossingPredecessor { return _impl._face_crossing_predecessor; }
};
