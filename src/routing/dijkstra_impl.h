...
dijkstra<G, Queue, U, L>::dijkstra(dijkstra<G, Queue, U, L> &&other) noexcept
        : _M_adj_list(other._M_adj_list), _M_labels(std::move(other._M_labels)), _M_graph(std::move(other._M_graph)),
...
template<RoutableGraph G, DijkstraQueue<G> Queue, typename U, DijkstraLabels L>
bool
dijkstra<G, Queue, U, L>::reached(const G::node_id_type &node) const {
