...
void Client::ClientModel<GraphT, RoutingT>::write_info(std::ostream &output) const {
    output << "path: " << result.route << '\n';
    output << "path has cost: " << graph.path_length(result.route) << " with beeline distance " << result.beeline_distance << '\n';
    output << "searches visited " << result.trees.nodes.size() << " nodes ";
    output << "and took " << result.duration << '\n';
}

template<typename GraphT, typename RoutingT> requires std::convertible_to<typename RoutingT::graph_type, GraphT>
void Client::ClientModel<GraphT, RoutingT>::write_beeline( std::ostream &output) const {
    std::vector<node_t> nodes = {graph.node(result.query.from), graph.node(result.query.to)};
...
void Client::ClientModel<GraphT, RoutingT>::write_tree_file(std::ostream &output) const {
    ...
    gl_file_io::write(output, tree_graph, 3, 4);
}
...
void Client::ClientModel<GraphT, RoutingT>::write_route_file(std::ostream &output) const {
    auto route_subgraph = graph.make_subgraph(result.route);
    auto route_graph = std_graph_t::make_graph(graph, route_subgraph);
    gl_file_io::write(output, route_graph, 1, 5);
}
...
Client::ClientModel<GraphT, RoutingT>::write_graph_stats(std::ostream &output) const {
    ...
    output << "\tgraph: expected size per node: "
           ...
           << graph.node_count() * steiner_graph::SIZE_PER_NODE / 1024 / 1024 << "MiB" << " + "
           << graph.edge_count() * steiner_graph::SIZE_PER_EDGE / 1024 / 1024 << "MiB"
           << std::endl;
