...
test_read_toy_fmi(const std::string &path) {
    ...
    auto c = [&](const std_graph_t &g) {
        ...
        assert_adjacency_list_equal(g.topology(), node_count, edge_count, expected_edges_fwd);
        assert_adjacency_list_equal(g.inverse_topology(), node_count, edge_count, expected_edges_bwd);
        return true;
    };
