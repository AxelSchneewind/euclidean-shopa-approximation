...
test_route_toy_fmi(std::string __path) {
    ...
    std::ifstream input;
    input.open(filename);
    auto graph = std::make_shared<std_graph_t>(
            std_graph_t(fmi_file_io::read<std_graph_t>(input)));
...
test_route_toy_ch(std::string path) {
    ...
    auto graph = std::make_shared<ch_graph_t>(ch_graph_t(fmi_file_io::read<ch_graph_t>(input)));
