...
struct steiner_neighbors_pruned_simplified {
private:
    Graph const &graph;
    Labels const &labels;
    ...
    void from_base_neighboring_node(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        for (unsigned char triangle_index = 0; triangle_index < 2; triangle_index++) [[unlikely]] {
            ...
            for (auto base_edge_id: triangle_edges) [[likely]] {
                ...
                short j = destination_steiner_info.node_count - 2;
                for (; j > i; --i) [[likely]] {
...
public:
    steiner_neighbors_pruned_simplified(Graph const &graph, Labels const& labels) : graph(graph), labels(labels), max_angle{std::atan(1.0 * graph.epsilon())}, max_angle_cos{std::cos(max_angle)} {}

    template<typename... Args>
    steiner_neighbors_pruned_simplified(Graph const &graph, Labels const& labels, Args const&...) : steiner_neighbors_pruned_simplified{graph, labels} {}
