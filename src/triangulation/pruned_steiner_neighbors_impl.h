...
private:
    ...
    void from_base_node(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        for (auto &&e: graph.base_graph().outgoing_edges(base_node_id)) [[likely]] {
            ...
            steiner_graph::node_id_type const destination{e_id, 1};
        ...
        for (auto &&e: graph.base_graph().incoming_edges(base_node_id)) [[likely]] {
            ...
            steiner_graph::node_id_type const destination{e_id, graph.steiner_info(e_id).node_count - 2U};
    ...
    void from_base_neighboring_node(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        for (unsigned char triangle_index = 0; triangle_index < 2; triangle_index++) [[unlikely]] {
            ...
            for (auto base_edge_id: triangle_edges) [[likely]] {
                ...
                for (typename Graph::node_id_type::intra_edge_id_type i = 0;
                     i < destination_steiner_info.node_count - 1; ++i) [[likely]] {
    ...
    void from_edge_using_segment(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        if (reached_from_other_edge.edge != node_id.edge) [[likely]] {
            auto visited_triangles = graph.base_polyhedron().edge_faces(reached_from_other_edge.edge);
        ...
        for (unsigned char triangle_index = triangle_first;
            ...
            for (auto base_edge_id: triangle_edges) [[likely]] {
                ...
                for (typename Graph::node_id_type::intra_edge_id_type i = 0;
                    ...
                    if (angle(from_other_edge_coordinate, source_coordinate, source_coordinate,
                              destination_coordinate) < max_angle
                        || angle(from_coordinate, source_coordinate, source_coordinate, destination_coordinate) <
                           max_angle) {
                        out.emplace_back(destination, node_id, 0);
                        destination_coordinates.emplace_back(destination_coordinate);
                    }
...
public:
    ~pruned_steiner_neighbors() = default;

    pruned_steiner_neighbors(pruned_steiner_neighbors &&) noexcept = default;

    pruned_steiner_neighbors &operator=(pruned_steiner_neighbors &&) noexcept = default;
    ...
    void operator()(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        // Case 1
        if (graph.is_base_node(__node_id))
            [[unlikely]]
