...
private:
    ...
    min_angle_neighbor(typename Graph::base_topology_type::edge_id_type const &edge_id, double const &max_angle_cos,
    ...
    add_min_angle_neighbor(NodeCostPair const &node, typename Graph::base_topology_type::edge_id_type const &edge_id,
        ...
        auto next = min_angle_neighbor<NodeCostPair>(edge_id, max_angle_cos, direction);
        if (is_none(next)) [[unlikely]]
            return next;

        // add edge with minimal angle
        coordinate_t const destination_coordinate{graph.node(next).coordinates};
    ...
    void from_base_node(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        if constexpr (steiner_graph::face_crossing_from_base_nodes) {
            ...
            auto &&triangle_edges = graph.base_polyhedron().node_edges(base_node_id);
            for (auto base_edge_id: triangle_edges) [[likely]] {
                epsilon_spanner(node, base_edge_id, max_angle_cos, _source_coordinate, out);
            }
        }
    }
    ...
    void from_steiner_node(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        while (face_crossing_predecessor != other_face_crossing_predecessor &&
               (!graph.is_base_node(face_crossing_predecessor) ||
                face_crossing_predecessor.edge == node_id.edge)) [[likely]] {
        ...
        for (unsigned char triangle_index = triangle_first;
            ...
            for (auto &&base_edge_id: triangle_edges) [[likely]] {
                ...
                add_min_angle_neighbor(node, base_edge_id, max_angle_cos, direction, out);

                if (face_crossing_predecessor != reached_from) [[unlikely]] {
                    add_min_angle_neighbor(node, base_edge_id, max_angle_cos,
                                           _source_coordinate - graph.node(reached_from).coordinates, out);
                }
            }
        }
    }
...
public:
    steiner_neighbors(Graph const &graph, Labels const &labels)
            : graph(graph), labels(labels),
              ...
              max_angle{std::clamp(M_PI_2 * graph.epsilon() * 1.0625, spanner_angle, M_PI_2)},
    ...
    void operator()(NodeCostPair const &node, std::vector<NodeCostPair> &out) {
        ...
        if (graph.is_base_node(node_id)) [[unlikely]] {
            auto base_node_id = graph.base_node_id(node_id);
            if (graph.is_boundary_node(base_node_id))
                [[unlikely]]
