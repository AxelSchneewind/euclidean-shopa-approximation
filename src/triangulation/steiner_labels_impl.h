...
template<RoutableGraph G, typename N>
bool
steiner_labels<G, N>::reached(steiner_labels<G, N>::node_id_type node) const {
    assert(!is_none(node));

    if (_M_graph.is_base_node(node)) [[unlikely]]
        return _M_base_labels[_M_graph.base_node_id(node)].distance != infinity<distance_t>;
    return _M_labels.node_info(node.edge, node.steiner_index - 1).distance != infinity<distance_t>;
}
